...
class Example(object):
    def __init__(self, src_sent, tgt_actions, tgt_code, tgt_ast, idx=0, meta=None, future_actions=None, relative_positions=None):
...
class Batch(object):
    def __init__(self, examples, grammar, vocab, copy=True, cuda=False, position_max=5):
        self.examples = examples
        ...
        self.copy = copy
        self.cuda = cuda
        self.position, self.pos_type = self.get_position_weight()
        self.init_index_tensors()
        self.init_future_index_tensors()
        token_mask = (self.primitive_copy_mask.transpose(0,1) + self.gen_token_mask.transpose(0,1)).bool()
        self.node_type = (self.apply_rule_mask.transpose(0,1)+2*token_mask.int()).long()

    def __len__(self):
        return len(self.examples)

    def get_position_weight(self):
        pos_max = self.position_max
        max_num = self.max_action_num
        weights = []
        types = []
        for e in self.examples:
            ...
            for i in range(max_num):
                tok_weight = []
                tok_type = []
                if i >= len(e.relative_positions):
                    tok_weight.extend([0.0] * max_num)
                    tok_type.extend([0.0] * max_num)
                else:
                    for pos in e.relative_positions[i]:
                        if pos > pos_max+0.5:
                ...
                weight.append(tok_weight)
                e_type.append(tok_type)
            weights.append(weight)
            types.append(e_type)
        return Variable(torch.cuda.LongTensor(weights)) if self.cuda else Variable(torch.LongTensor(weights)),\
            Variable(torch.cuda.LongTensor(types)) if self.cuda else Variable(torch.LongTensor(types))

    def get_frontier_field_idx(self, t, future=False):
        ids = []
        for e in self.examples:
            if not future:
                ...
                if t < len(e.tgt_actions):
                    if e.future_actions[t] == '<none>':
                        ids.append(len(self.grammar.fields))
                    elif e.future_actions[t] == '<reduce_none>':
                        ids.append(self.grammar.field2id[e.tgt_actions[t].frontier_field])
                    else:
                        ids.append(self.grammar.field2id[e.future_actions[t].frontier_field])
                else:
                    ids.append(0)

        return Variable(torch.cuda.LongTensor(ids)) if self.cuda else Variable(torch.LongTensor(ids))

    def get_future_parent_state(self, t, history_states, hidden_size, zero_parent_state):
        states = []
        for batch_id, e in enumerate(self.examples):
            if t < len(e.tgt_actions):
                if e.future_actions[t] == '<none>':
                    states.append(zero_parent_state)
                elif e.future_actions[t] == '<reduce_none>':
                    states.append(history_states[e.tgt_actions[t].parent_t][0][batch_id])
                else:
                    states.append(history_states[e.future_actions[t].parent_t][0][batch_id])

            else:
                states.append(history_states[0][0][batch_id])
        return torch.stack(states)

    def get_frontier_prod_idx(self, t, future=False):
        ids = []
        for e in self.examples:
            if not future:
                if t < len(e.tgt_actions):
                ...
                if t < len(e.tgt_actions):
                    if e.future_actions[t] == '<none>':
                        ids.append(len(self.grammar)+1)
                    elif e.future_actions[t] == '<reduce_none>':
                        ids.append(self.grammar.prod2id[e.tgt_actions[t].frontier_prod])
                    else:
                        ids.append(self.grammar.prod2id[e.future_actions[t].frontier_prod])
    ...
    def get_frontier_field_type_idx(self, t, future=False):
        ids = []
        for e in self.examples:
            if not future:
                if t < len(e.tgt_actions):
                ...
                if t < len(e.tgt_actions) and e.future_actions[t] != '<none>':
                    ids.append(self.grammar.type2id[e.future_actions[t].frontier_field.type])
                else:
                    ids.append(0)
    ...
    def init_future_index_tensors(self):
        self.future_apply_rule_idx_matrix = []
        self.future_apply_rule_mask = []
        self.future_primitive_idx_matrix = []
        self.future_gen_token_mask = []
        self.future_primitive_copy_mask = []
        self.future_primitive_copy_token_idx_mask = np.zeros((self.max_action_num, len(self), max(self.src_sents_len)), dtype='float32')
        self.future_field_ids = []
        ...
        for t in range(self.max_action_num):
            ...
            for e_id, e in enumerate(self.examples):
                app_rule_idx = app_rule_mask = token_idx = gen_token_mask = copy_mask = 0
                if t < len(e.tgt_actions):
                    action_info = e.future_actions[t]
                    if action_info == '<none>' or action_info == '<reduce_none>':
                        action = '<none>'
                    else:
                        action = e.future_actions[t].action
                    ...
                    if isinstance(action, ApplyRuleAction):
                        ...
                        app_rule_idx = len(self.grammar) + 1
                        app_rule_mask = 1
                    else:
                        src_sent = self.src_sents[e_id]
                        token = str(action.token)
                        ...
                        if self.copy and token in src_sent:
                            token_pos_list = [idx for idx, _token in enumerate(src_sent) if _token == token]
                            self.future_primitive_copy_token_idx_mask[t, e_id, token_pos_list] = 1.
                            copy_mask = 1
                            token_can_copy = True
            ...
            self.future_apply_rule_idx_matrix.append(app_rule_idx_row)
            self.future_apply_rule_mask.append(app_rule_mask_row)

            self.future_primitive_idx_matrix.append(token_row)
            self.future_gen_token_mask.append(gen_token_mask_row)

            self.future_primitive_copy_mask.append(copy_mask_row)

        T = torch.cuda if self.cuda else torch
        self.future_apply_rule_idx_matrix = Variable(T.LongTensor(self.future_apply_rule_idx_matrix))
        self.future_apply_rule_mask = Variable(T.FloatTensor(self.future_apply_rule_mask))
        self.future_primitive_idx_matrix = Variable(T.LongTensor(self.future_primitive_idx_matrix))
        self.future_gen_token_mask = Variable(T.FloatTensor(self.future_gen_token_mask))
        self.future_primitive_copy_mask = Variable(T.FloatTensor(self.future_primitive_copy_mask))
        self.future_primitive_copy_token_idx_mask = Variable(torch.from_numpy(self.future_primitive_copy_token_idx_mask))
        if self.cuda: self.future_primitive_copy_token_idx_mask = self.future_primitive_copy_token_idx_mask.cuda()
