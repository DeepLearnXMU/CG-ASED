...
@Registrable.register('parser_tree')
class ParserTree(nn.Module):
    ...
    def __init__(self, args, vocab, transition_system):
        super(ParserTree, self).__init__()

        self.args = args
        self.vocab = vocab
        self.pos_max = args.pos_max
        ...
        # the last two entries are the embedding for Reduce action and <none> future action
        ...
        if args.no_copy is False:
            ...
            self.primitive_predictor2 = nn.Linear(args.att_vec_size, 2)

        if args.primitive_token_label_smoothing:
            self.label_smoothing = LabelSmoothing(args.primitive_token_label_smoothing, len(self.vocab.primitive),
        ...
        self.att_vec_linear = nn.Linear(args.hidden_size *4 , args.att_vec_size, bias=True)

        # past module
        self.dec_att_linear = nn.Linear(args.hidden_size + args.hidden_size, args.hidden_size, bias=False)
        self.K = nn.Linear(args.hidden_size, args.hidden_size, bias=False)
        # embedding table for relative position
        self.K_rel_pos = nn.Embedding(self.pos_max+1, args.hidden_size)
        self.K_pos_type = nn.Embedding(3, args.hidden_size)
        self.K_node_type = nn.Embedding(3, args.hidden_size)

        # future module
        self.future_linear = nn.Linear(args.hidden_size + args.hidden_size + args.field_embed_size, args.hidden_size, bias=False)
        self.att_future_vec_linear = nn.Linear(args.hidden_size + args.hidden_size, args.att_vec_size, bias=False)
        self.att_all_vec_linear = nn.Linear(args.hidden_size * 3, args.hidden_size, bias=False)
        self.fuse_future_linear = nn.Linear(args.hidden_size + args.hidden_size, args.hidden_size, bias=True)
        self.fuse_future_linear2 = nn.Linear(args.hidden_size, args.hidden_size, bias=True)
        ...
        self.zero_parent_state = nn.Parameter(torch.Tensor(args.hidden_size))
        self.init_hidden = nn.Parameter(torch.Tensor(1, args.hidden_size))
        self.span_b = nn.Linear(args.hidden_size, 1)
        nn.init.constant_(self.span_b.bias.data, -1)
        ...
        self.future_dp = nn.Dropout(args.future_dp)
        self.past_dp = nn.Dropout(args.past_dp)
        self.new_dp = nn.Dropout(0.3)
    ...
    def classfier(self, query_vectors, src_encodings, batch):
    ...
    def future_classfier(self, query_vectors, src_encodings, batch):
        # ApplyRule (i.e., ApplyConstructor) action probabilities
        # (tgt_action_len, batch_size, grammar_size)
        apply_rule_prob = F.softmax(self.production_readout(query_vectors), dim=-1)
        ...
        # (tgt_action_len, batch_size)
        tgt_apply_rule_prob = torch.gather(apply_rule_prob, dim=2,
                                           index=batch.future_apply_rule_idx_matrix.unsqueeze(2)).squeeze(2)
        ...
        # (tgt_action_len, batch_size)
        tgt_primitive_gen_from_vocab_prob = torch.gather(gen_from_vocab_prob, dim=2,
                                                         index=batch.future_primitive_idx_matrix.unsqueeze(2)).squeeze(2)
        ...
        if self.args.no_copy:
            ...
            if self.training and self.args.primitive_token_label_smoothing:
                ...
                tgt_primitive_gen_from_vocab_log_prob = -self.label_smoothing(
                    gen_from_vocab_prob.log(),
                    batch.future_primitive_idx_matrix)
            else:
                tgt_primitive_gen_from_vocab_log_prob = tgt_primitive_gen_from_vocab_prob.log()

            # (tgt_action_len, batch_size)
            action_prob = tgt_apply_rule_prob.log() * batch.future_apply_rule_mask + \
                          tgt_primitive_gen_from_vocab_log_prob * batch.future_gen_token_mask
        else:
            # binary gating probabilities between generating or copying a primitive token
            # (tgt_action_len, batch_size, 2)
            primitive_predictor = F.softmax(self.primitive_predictor(query_vectors), dim=-1)
            ...
            # marginalize over the copy probabilities of tokens that are same
            # (tgt_action_len, batch_size)
            tgt_primitive_copy_prob = torch.sum(primitive_copy_prob * batch.future_primitive_copy_token_idx_mask, dim=-1)

            # mask positions in action_prob that are not used
            # (tgt_action_len, batch_size)
            action_mask_pad = torch.eq(batch.future_apply_rule_mask + batch.future_gen_token_mask + batch.future_primitive_copy_mask, 0.)
            action_mask = 1. - action_mask_pad.float()

            # (tgt_action_len, batch_size)
            action_prob = tgt_apply_rule_prob * batch.future_apply_rule_mask + \
                          primitive_predictor[:, :, 0] * tgt_primitive_gen_from_vocab_prob * batch.future_gen_token_mask + \
                          primitive_predictor[:, :, 1] * tgt_primitive_copy_prob * batch.future_primitive_copy_mask
    ...
    def score(self, examples, return_encode_state=False):
        ...
        if self.args.sup_attention:
            query_vectors, future_vectors, att_prob = self.decode(batch, src_encodings, dec_init_vec)
        else:
            query_vectors, future_vectors = self.decode(batch, src_encodings, dec_init_vec)

        scores = self.classfier(query_vectors, src_encodings, batch)
        future_scores = self.future_classfier(future_vectors, src_encodings, batch)

        returns = [scores + self.alpha * future_scores]
        if self.args.sup_attention:
    ...
    def decoder_attention(self, h_t, history_hidden, position, pos_type, node_type):
        """
        :param h_t: (batch_size, hidden_size)
        :param history_hidden: (batch_size, tgt_len, hidden_size)
        :param position: (batch_size, tgt_len)
        """
        # history_h: (batch_size, history_len, hidden_size)
        history_len = history_hidden.shape[1]
        position = position[:, :history_len]
        pos_type = pos_type[:, :history_len]
        node_type = node_type[:, :history_len]

        # (batch_size, tgt_len)
        init_score = torch.bmm(self.K(history_hidden) + self.K_rel_pos(position) + \
                               self.K_pos_type(pos_type) + self.K_node_type(node_type),
                               h_t.unsqueeze(2)).squeeze(2)

        pad_matrix = ~position.bool()
        s = 5 * torch.sigmoid(self.span_b(h_t))
        rescale_weight = torch.clamp(((3 + s - position)/3), 0, 1)

        init_score = (init_score - torch.max(init_score, -1)[0].unsqueeze(-1)).exp() * rescale_weight
        att_weight = init_score / init_score.sum(dim=-1, keepdim=True)

        # for the pad step, pad with zero
        att_weight.data.masked_fill_(pad_matrix, 0)
        att_weight.data.masked_fill_(~rescale_weight.bool(), 0)

        # (batch_size, hidden_size)
        ctx_vec = torch.bmm(att_weight.unsqueeze(1), history_hidden).squeeze(1)
        return ctx_vec, att_weight

    def step(self, x, h_tm1, history_hidden, position, pos_type, node_type,
             future_parent_production_embed, future_field_embed, src_encodings, src_encodings_att_linear,
        ...
        Args:
            ...
            future_parent_production_embed: variable of shape (batch_size, hidden_size), future input
        ...
        if isinstance(history_hidden, list):
            history_hidden = h_t.unsqueeze(1)
            past_h = torch.zeros_like(h_t)
        else:
            dec_ctx, _ = self.decoder_attention(h_t, history_hidden, position, pos_type, node_type)
            history_hidden = torch.cat([history_hidden, h_t.unsqueeze(1)], 1)
            past_h = dec_ctx
        past_h = self.past_dp(past_h)
        ctx_t, alpha_t = nn_utils.dot_prod_attention(h_t,
                                                     src_encodings, src_encodings_att_linear,
                                                     mask=src_token_mask)
        # get the future att_t
        future_input = self.future_linear(
            torch.cat([h_t, future_parent_production_embed, future_field_embed], 1))
        future_ctx, _ = nn_utils.dot_prod_attention(future_input,
                                                    src_encodings, src_encodings_att_linear,
                                                    mask=src_token_mask)
        future_att = torch.tanh(self.att_future_vec_linear(torch.cat([future_input, future_ctx], 1)))
        future_att = self.future_dp(future_att)

        new_h = torch.sigmoid(self.fuse_future_linear(torch.cat([h_t, future_att], 1)))
        future_h = torch.tanh(self.fuse_future_linear2(new_h * future_att))


        att_t = torch.tanh(self.att_vec_linear(torch.cat([h_t, past_h, ctx_t, future_h], 1)))  # E.q. (5)
        att_t = self.dropout(att_t)

        if return_att_weight:
            return (h_t, cell_t), att_t, history_hidden, future_att, alpha_t
        else:
            return (h_t, cell_t), att_t, history_hidden, future_att
    ...
    def decode(self, batch, src_encodings, dec_init_vec):
        ...
        zero_parent_state = self.zero_parent_state
        ...
        for t in range(batch.max_action_num):
            ...
            # get the future action's parent embedding
            future_parent_state = batch.get_future_parent_state(t, history_states, args.hidden_size, zero_parent_state)
            future_field_embed = self.field_embed(batch.get_frontier_field_idx(t, future=True))

            (h_t, cell_t), att_t, history_hidden, future_att, att_weight = self.step(x, h_tm1, history_hidden,
                                                                     batch.position[:, t],
                                                                     batch.pos_type[:, t],
                                                                     batch.node_type,
                                                                     future_parent_state,
            ...
            att_future_vecs.append(future_att)
        ...
        att_future_vecs = torch.stack(att_future_vecs, dim=0)
        if args.sup_attention:
            return att_vecs, att_future_vecs, att_probs
        else:
            return att_vecs, att_future_vecs
    ...
    def parse(self, src_sent, context=None, beam_size=5, debug=False):
        ...
        while len(completed_hypotheses) < beam_size and t < args.decode_max_time_step:
            ...
            def get_position_weight(relative_positions):
                    ...
                    for pos in relative_position:
                        if pos > pos_max+0.5:
                            ...
                            # other
                                ...
                                tok_type.append(2)
                            tok_weight.append(int(pos))
                            #tok_weight.append(1.0 - float(pos) / pos_max)
                    pad_len = max_num - len(tok_weight)
                    tok_weight.extend([0.0] * pad_len)
                    position.append(tok_weight)
                    tok_type.extend([0.0] * pad_len)
                    types.append(tok_type)
                return Variable(torch.cuda.LongTensor(position)) if self.cuda else Variable(torch.LongTensor(position)),\
                      Variable(torch.cuda.LongTensor(types)) if self.cuda else Variable(torch.LongTensor(types))

            relative_positions = []
            for index, hyp in enumerate(hypotheses):
                position = []
                if hyp.frontier_node is None or hyp.frontier_node.created_time == -1:
                    position.append(0)
                else:
                    # before parent node, position = parent-to-goal position + 1
                    for pos in hyp_pos[index][hyp.frontier_node.created_time]:
                        position.append(pos + 1)
                    # after parent node, position = goal-to-parent position + 1
                    for i in range(len(position), t):
                        position.append(1.5 + hyp_pos[index][i][hyp.frontier_node.created_time])
                    position.append(0)
                relative_positions.append(position)
                hyp_pos[index].append(position)

            if t == 0:
                node_types = Variable(self.new_tensor(hyp_num, 1).zero_())
            else:
                node_types = Variable(torch.cuda.LongTensor(hyp_node_type))

            # get the future action's parent embedding
            def get_next(current_field, node):
                # if current field is last, future node equal parent's future node
                cur_id = node.fields.index(current_field)
                if current_field.cardinality is 'multiple':
                    return current_field
                elif len(node.fields) <= 1 or cur_id == len(node.fields)-1:
                    return None
                else:
                    return node.fields[cur_id + 1]

            for i, hyp in enumerate(hypotheses):
                if hyp.frontier_node is None or hyp.frontier_node.created_time == -1:
                    hyp_future_node[i].append(-1)
                    hyp_future_field[i].append(-1)
                else:
                    next_field = get_next(hyp.frontier_field, hyp.frontier_node)
                    if next_field:
                        hyp_future_node[i].append(hyp.frontier_node.created_time)
                        hyp_future_field[i].append(self.grammar.field2id[next_field.field])
                    else:
                        hyp_future_node[i].append(hyp_future_node[i][hyp.frontier_node.created_time])
                        hyp_future_field[i].append(hyp_future_field[i][hyp.frontier_node.created_time])

                # print(hyp_future_node[i][-1])

            future_parent_states = torch.stack([hyp_states[hyp_id][p_t[-1]][0] if p_t[-1] != -1 else zero_parent_state
                                                for hyp_id, p_t in enumerate(hyp_future_node)])

            future_field_embed = self.field_embed(Variable(self.new_long_tensor(
                [f[-1] if f[-1] != -1 else len(self.grammar.fields) for f in hyp_future_field])))

            positions, pos_types = get_position_weight(relative_positions)
            (h_t, cell_t), att_t, history_hidden, _ = self.step(x, h_tm1, history_hidden, positions,
                                             pos_types, node_types,
                                             future_parent_states,
                                             future_field_embed,
                                             exp_src_encodings,
            ...
            for new_hyp_score, new_hyp_pos in zip(top_new_hyp_scores.data.cpu(), top_new_hyp_pos.data.cpu()):
                ...
                if new_hyp_pos < len(applyrule_new_hyp_scores):
                    ...
                    if debug:
                        action_info.gen_copy_switch = 'n/a' if args.no_copy else primitive_predictor_prob[prev_hyp_id,
                                                                                 :].log().cpu().data.numpy()
                ...
                if new_hyp.completed:
                    ...
                    new_node_type.append(node_type)

            if live_hyp_ids:
                hyp_states = [hyp_states[i] + [(h_t[i], cell_t[i])] for i in live_hyp_ids]
                hyp_future_node = [deepcopy(hyp_future_node[i]) for i in live_hyp_ids]
                hyp_future_field = [deepcopy(hyp_future_field[i]) for i in live_hyp_ids]
                h_tm1 = (h_t[live_hyp_ids], cell_t[live_hyp_ids])
                hyp_pos = [deepcopy(hyp_pos[i]) for i in live_hyp_ids]
                hyp_node_type = [deepcopy(hyp_node_type[i])+ [new_node_type[e]] for e, i in enumerate(live_hyp_ids)]
                att_tm1 = att_t[live_hyp_ids]
                history_hidden = history_hidden[live_hyp_ids]
