    ...
    def parse_django_dataset(annot_file, code_file, asdl_file_path, max_query_len=70, vocab_freq_cutoff=10):
        ...
        for idx, (src_query, tgt_code) in enumerate(zip(open(annot_file), open(code_file))):
            ...
            for index, action in enumerate(tgt_action_infos):
                if action.parent_t == -1:
                    ...
                    # if occur the reduce action, we regard it as primitive type
                    #if action.frontier_field.cardinality is 'multiple':
            ...
            if len(get_action_infos(src_query_tokens,loaded_examples[-1]["tgt_actions"]))>15:
                continue
            #print("id:",idx)
            #print("src_sent:",src_query_tokens)
            #print("tgt_actions:",get_action_infos(src_query_tokens,loaded_examples[-1]["tgt_actions"]))
            #print("tgt_code:",loaded_examples[-1]['tgt_canonical_code'])
            #print("tgt_ast:",loaded_examples[-1]['tgt_ast'])
            #print('raw_code:', loaded_examples[-1]['raw_code'],"\n", 'str_map:',loaded_examples[-1]['str_map'])
            #print("**********************************************")
        ...
        for idx, e in enumerate(loaded_examples):
            ...
            example = Example(idx=idx,
                              ...
                              future_actions=e['future_actions'],
                              relative_positions=e['relative_positions'])
        ...
        print('Max action len: %f' % max(action_len), file=sys.stderr)
        print('Avg action len: %f' % np.average(action_len), file=sys.stderr)
        print('Actions larger than 100: %d' % len(list(filter(lambda x: x > 100, action_len))), file=sys.stderr)

        #action_deep = [deep[0] for e in loaded_examples for deep in e['relative_positions']]
        action_deep=[]
        for e in loaded_examples:
            action_deep.append(max([deep[0] for deep in e['relative_positions']]))
        print('Max action deep: %f' % max(action_deep), file=sys.stderr)
        print('Avg action deep: %f' % np.average(action_deep), file=sys.stderr)
